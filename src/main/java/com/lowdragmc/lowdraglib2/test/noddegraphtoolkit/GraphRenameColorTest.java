...
public class GraphRenameColorTest {
    ...
    public static void abstractNodeModelColorStoragePersisted(GameTestHelper helper) {
        ...
        if (add.hasUserColor()) {
            helper.fail("fresh node should not have user color");
            return;
        }
        ...
        if (!add.hasUserColor()) {
            helper.fail("setColor should mark userColor");
            return;
        }
    ...
    public static void renamableJudgmentRespectsCapabilityAndInterface(GameTestHelper helper) {
        ...
        // Renamable + IHasName
        if (!(pm.isRenamable() && pm instanceof IHasName)) {
            helper.fail("Placemat renamable");
            return;
        }
        if (!(subNode.isRenamable() && subNode instanceof IHasName)) {
            helper.fail("Subgraph renamable");
            return;
        }
        if (!(variable.isRenamable() && variable instanceof IHasName)) {
            helper.fail("Variable renamable");
            return;
        }

        // NOT renamable
        if (addNode.isRenamable()) {
            helper.fail("TestAddNode should not be renamable");
            return;
        }
        // StickyNote: not renamable (per design — content is the body)
        if (sn.isRenamable()) {
            helper.fail("StickyNote should not be renamable");
            return;
        }
    ...
    public static void colorableSettersRoundTrip(GameTestHelper helper) {
        ...
        if (!(pm instanceof IHasElementColor)) {
            helper.fail("Placemat should be IHasElementColor");
            return;
        }
        pm.setColor(0xFFAA5500);
        if (pm.getElementColor() != 0xFFAA5500) {
            helper.fail("placemat color not stored");
            return;
        }
        if (!pm.hasUserColor()) {
            helper.fail("placemat userColor flag not raised");
            return;
        }
    ...
    public static void wireIsNeitherRenamableNorColorable(GameTestHelper helper) {
        ...
        if (wire.isRenamable()) {
            helper.fail("wire should not be renamable");
            return;
        }
        if (wire.isColorable()) {
            helper.fail("wire should not be colorable");
            return;
        }
    ...
    public static void resetColorRestoresDefault(GameTestHelper helper) {
        ...
        if (!add.hasUserColor() || add.getElementColor() != 0xFF112233) {
            helper.fail("precondition: node setColor failed");
            return;
        }
        add.resetColor();
        if (add.hasUserColor()) {
            helper.fail("node hasUserColor still true after resetColor");
            return;
        }
        if (add.getElementColor() != nodeDefault) {
            helper.fail("node elementColor not back to default after reset: 0x" + Integer.toHexString(add.getElementColor()));
            return;
        }
        ...
        if (!pm.hasUserColor() || pm.getElementColor() != 0xFF99AA66) {
            helper.fail("precondition: placemat setColor failed");
            return;
        }
        pm.resetColor();
        if (pm.hasUserColor() || pm.getElementColor() != pmDefault) {
            helper.fail("placemat reset did not restore defaults");
            return;
        }
        ...
        pm.resetColor();
        if (pm.hasUserColor()) {
            helper.fail("idempotent reset broke state");
            return;
        }
        ...
        for (var n : graph2.graphModel.getNodeModels()) {
            if (n != null && n.getUid().equals(add.getUid())) {
                if (n.hasUserColor()) {
                    helper.fail("hasUserColor leaked through round-trip after reset");
                    return;
                }
                break;
            }
