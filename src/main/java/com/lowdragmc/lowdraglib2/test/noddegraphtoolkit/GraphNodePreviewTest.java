...
public class GraphNodePreviewTest {
    ...
    public static void previewAutoCreatedAndDependent(GameTestHelper helper) {
        ...
        if (preview == null) {
            helper.fail("preview not auto-created for hasNodePreview node");
            return;
        }
        boolean inDeps = node.getDependentModels().anyMatch(m -> m == preview);
        if (!inDeps) {
            helper.fail("preview model missing from getDependentModels");
            return;
        }
        if (!preview.isExpanded()) {
            helper.fail("preview should default to expanded");
            return;
        }
        ...
        if (plain.getNodePreviewModel() != null) {
            helper.fail("non-preview node should report null preview");
            return;
        }
    ...
    public static void orphanSpawnSkipsPreview(GameTestHelper helper) {
        ...
        if (node.getNodePreviewModel() != null) {
            helper.fail("ORPHAN spawn should not create a preview");
            return;
        }
    ...
    public static void duplicateCopiesExpandedState(GameTestHelper helper) {
        ...
        if (dst.isPreviewExpanded()) {
            helper.fail("duplicated node should inherit collapsed (expanded=false) preview state");
            return;
        }
        if (dst.getNodePreviewModel() == null || dst.getNodePreviewModel().isExpanded()) {
            helper.fail("duplicated preview model should mirror expanded=false");
            return;
        }
    ...
    public static void expandedStatePersistsRoundTrip(GameTestHelper helper) {
        ...
        for (var n : graph2.graphModel.getNodeModels()) {
            if (n != null && n.getUid().equals(node.getUid())) {
                restored = n;
                break;
            }
        }
        if (restored == null) {
            helper.fail("preview node not found after deserialize");
            return;
        }
        if (restored.getNodePreviewModel() == null) {
            helper.fail("preview model not recreated on load (syncNodePreview)");
            return;
        }
        if (restored.isPreviewExpanded()) {
            helper.fail("expanded=false not persisted across round-trip");
            return;
        }
        if (restored.getNodePreviewModel().isExpanded()) {
            helper.fail("restored preview model expanded state not synced from node");
            return;
        }
    ...
    public static void previewDefaultExpandedApi(GameTestHelper helper) {
        ...
        if (!defaultExpanded.isPreviewExpanded()) {
            helper.fail("preview should default to expanded when node does not override the API");
            return;
        }
        if (defaultExpanded.getNodePreviewModel() == null || !defaultExpanded.getNodePreviewModel().isExpanded()) {
            helper.fail("preview model should mirror default expanded state");
            return;
        }
        ...
        if (defaultCollapsed.isPreviewExpanded()) {
            helper.fail("preview should default to collapsed when node overrides the API");
            return;
        }
        if (defaultCollapsed.getNodePreviewModel() == null || defaultCollapsed.getNodePreviewModel().isExpanded()) {
            helper.fail("preview model should mirror default collapsed state");
            return;
        }
    ...
    public static void previewExpandedStateOverridesDefaultOnLoad(GameTestHelper helper) {
        ...
        if (restored == null) {
            helper.fail("collapsed-default preview node not found after deserialize");
            return;
        }
        if (!restored.isPreviewExpanded()) {
            helper.fail("persisted expanded=true should override default collapsed state");
            return;
        }
        if (restored.getNodePreviewModel() == null || !restored.getNodePreviewModel().isExpanded()) {
            helper.fail("restored preview model should mirror persisted expanded=true");
            return;
        }
    ...
    public static void duplicatePreviewStateOverridesDefault(GameTestHelper helper) {
        ...
        if (!dst.isPreviewExpanded()) {
            helper.fail("duplicated preview should inherit source expanded=true over default collapsed");
            return;
        }
        if (dst.getNodePreviewModel() == null || !dst.getNodePreviewModel().isExpanded()) {
            helper.fail("duplicated preview model should mirror inherited expanded=true");
            return;
        }
