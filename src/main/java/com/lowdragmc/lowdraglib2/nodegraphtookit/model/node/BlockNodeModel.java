 ...
 *
 * <p>
 * Block models are <em>not</em> added to {@code GraphModel#nodeModels}. Their lifecycle is
 * owned by their parent context, but their ports are still registered with the graph (via the
 * context's {@code getDependentModels()}) so wires can target them.
 * </p>
 *
 * <p>
 * This base owns block semantics (parent link, "needs container" capability). The
 * user-node binding lives in {@link CustomBlockNodeModelImpl}, mirroring the
 * {@link NodeModel} / {@link CustomNodeModelImpl} split.
 * </p>
 *
 * <p>
 * {@link #createElementUI()} returns {@code null}: blocks are not created as top-level UI
