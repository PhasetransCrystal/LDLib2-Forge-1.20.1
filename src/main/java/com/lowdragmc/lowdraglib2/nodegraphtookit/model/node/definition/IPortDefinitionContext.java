...
public interface IPortDefinitionContext {
     ...
     *
     * <p>
     * {@code portId} is used to identify the port. It must be unique among input ports and node options on the node.
     * This name is used as the ID when calling {@link Node#getInputPortById(String)}.
     * If {@link IPortBuilder#withDisplayName(net.minecraft.network.chat.Component)} is not used, this name is also used
     * as the port's display label.
     * </p>
     *
     * <p>
     * <b>Warning:</b> Changing a port's name will break any existing connections, as the name is used as the port's
     * unique ID.
     * </p>
     *
     * <p>
     * Use the returned builder to configure port properties and then call {@link IPortBuilder#build()} to create the
     * port.
     * </p>
     *
     ...
     *
     * <p>
     * {@code portId} is used to identify the port. It must be unique among output ports on the node.
