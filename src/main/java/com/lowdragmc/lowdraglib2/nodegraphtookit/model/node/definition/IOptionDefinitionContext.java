 ...
 *
 * <p>
 * Use to add node options on nodes. Node options appear under the node header and in the inspector
 * when a node is selected. They are appropriate for parameters that affect how a node behaves or changes
 * its topology, such as modifying the number of ports.
 * </p>
 */
public interface IOptionDefinitionContext {
     ...
     *
     * <p>
     * {@code name} is used to identify the option. It must be unique among ports and options on the node.
     * This name is used as the ID when calling {@link Node#getNodeOptionById(String)}.
     * If {@link IOptionBuilder#withDisplayName(Component)} is not used, this name is also used as the option's display
     * label.
     * </p>
     *
