...
public record RPCEmitter(RPCEvent event, Supplier<@Nullable ModularUI> modularProvider) {
     ...
     * @return {@code true} if the event is successfully sent; {@code false} if the {@link ModularUI} instance is not
     *         available.
     */
     ...
     * @param callback a callback function to handle the response from the event. Can be {@code null} if no response is
     *                 expected.
     * @param args     the arguments to be sent with the event. The number and types of arguments
     *                 should match the requirements of the {@link RPCEvent}.
     * @return {@code true} if the event is successfully sent; {@code false} if the {@link ModularUI} instance is not
