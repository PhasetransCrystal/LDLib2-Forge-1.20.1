 ...
 */
open class BindableValueSpec<T, E : BindableValue<T>>(var initialValue: T? = null, var onValueChanged: Consumer<T>? = null) : ElementSpec<E>() {
    /**
 ...
 */
open class BindableValueElement<T, E : BindableValue<T>>(element: E, spec: (BindableValueSpec<T, E>.() -> Unit)? = null) : UIContainer<E, BindableValueSpec<T, E>>(element, spec) {
    override fun makeSpec(): BindableValueSpec<T, E>? = spec?.let { BindableValueSpec<T, E>().apply(it) }
 ...
 */
fun <T> bindableValue(initialValue: T? = null, spec: (BindableValueSpec<T, BindableValue<T>>.() -> Unit)? = null, init: BindableValueElement<T, BindableValue<T>>.() -> Unit = {}): BindableValue<T> = BindableValueElement(BindableValue(initialValue), spec).apply(init).build()

/**
 * Internal Builder - Add BindableValue as a child to a container
 */
fun <T> UIContainer<*, *>.bindableValue(initialValue: T? = null, spec: (BindableValueSpec<T, BindableValue<T>>.() -> Unit)? = null, init: BindableValueElement<T, BindableValue<T>>.() -> Unit = {}) = add(BindableValueElement(BindableValue(initialValue), spec), init)

/**
 * DSL converter - Convert existing BindableValue to DSL builder
 */
fun <T> BindableValue<T>.dsl(spec: (BindableValueSpec<T, BindableValue<T>>.() -> Unit)? = null, init: BindableValueElement<T, BindableValue<T>>.() -> Unit = {}): BindableValueElement<T, BindableValue<T>> = BindableValueElement(this, spec).apply(init)
 ...
 */
fun <T, E : BindableValue<T>> BindableValueElement<T, E>.getValue(): T = element.value
