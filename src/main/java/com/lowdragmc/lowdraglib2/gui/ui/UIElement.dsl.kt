...
@UIDslMarker
class EventsDsl internal constructor(private val pendingEvents: MutableMap<String, PendingEventOps>) {
    private fun ops(type: String): PendingEventOps = pendingEvents.getOrPut(type) { PendingEventOps() }
 ...
 */
open class ElementSpec<T : UIElement>(var id: String? = null, var cls: (ClassPatchDsl.() -> Unit)? = null, var layout: (TaffyLayoutStyleDsl.() -> Unit)? = null, var style: (BasicStyle.() -> Unit)? = null, var focusable: Boolean? = null, var visible: Boolean? = null, var active: Boolean? = null)

/**
 * Container for UI builder elements with children
 */
abstract class UIContainer<T : UIElement, S : ElementSpec<T>>(val element: T, val spec: (S.() -> Unit)? = null) : UIBuilder<T> {
    ...
    fun <T, D> UIContainer<T, *>.bindUIData(getter: () -> D, setter: (D) -> Unit) where T : UIElement, T : IObservable<D>, T : IDataConsumer<D> {
        dataSource(getter)
        observer(setter)
    }
    ...
    inline fun <T, reified D> UIContainer<T, *>.bind(noinline getter: () -> D, noinline setter: (D) -> Unit, initialValue: D? = null) where T : UIElement, T : IBindable<D> {
        (element as? IBindable<D>)?.bind(bindings(getter, setter, initialValue).build())
    }

    inline fun <T, reified D> UIContainer<T, *>.bind(prop: KMutableProperty0<D>, initialValue: D? = null) where T : UIElement, T : IBindable<D> {
 ...
 */
open class Element<T : UIElement>(element: T, spec: (ElementSpec<T>.() -> Unit)? = null) : UIContainer<T, ElementSpec<T>>(element, spec) {
    override fun makeSpec(): ElementSpec<T>? = spec?.let { ElementSpec<T>().apply(it) }
}

/**
 * Root UI builder function
 */
fun element(spec: (ElementSpec<UIElement>.() -> Unit)? = null, init: Element<UIElement>.() -> Unit): UIElement = Element(UIElement(), spec).apply(init).build()

fun UIContainer<*, *>.element(spec: (ElementSpec<UIElement>.() -> Unit)? = null, init: Element<UIElement>.() -> Unit = {}) = add(Element(UIElement(), spec), init)

fun UIContainer<*, *>.row(spec: (ElementSpec<UIElement>.() -> Unit)? = null, init: Element<UIElement>.() -> Unit = {}) = add(
    Element(
        UIElement().layoutDsl {
...
fun UIContainer<*, *>.column(spec: (ElementSpec<UIElement>.() -> Unit)? = null, init: Element<UIElement>.() -> Unit = {}) = add(
    Element(
        UIElement().layoutDsl {
            alignItems(AlignItems.FLEX_START)
        },
...
fun UIContainer<*, *>.inventorySlots(spec: (ElementSpec<InventorySlots>.() -> Unit)? = null, init: Element<InventorySlots>.() -> Unit = {}) = add(Element(InventorySlots(), spec), init)
...
fun <T : UIElement> T.dsl(spec: (ElementSpec<T>.() -> Unit)?, init: Element<T>.() -> Unit = {}): Element<T> = Element(this, spec).apply(init)
...
inline fun <reified T> bindings(noinline getter: () -> T, noinline setter: (T) -> Unit, initialValue: T? = null): DataBindingBuilder<T> = DataBindingBuilder.create(getter, setter).syncType(T::class.java).initialValue(initialValue)

inline fun <reified T> bindingsS2C(noinline getter: () -> T, initialValue: T? = null): DataBindingBuilder<T> = DataBindingBuilder.create(getter, LDConsumers.nop())
    .syncType(T::class.java)
    .c2sStrategy(SyncStrategy.NONE)
    .initialValue(initialValue)

inline fun <reified T> bindingsC2S(noinline setter: (T) -> Unit, initialValue: T? = null): DataBindingBuilder<T> = DataBindingBuilder.create(LDSuppliers.nul<T>(), setter)
    .syncType(T::class.java)
