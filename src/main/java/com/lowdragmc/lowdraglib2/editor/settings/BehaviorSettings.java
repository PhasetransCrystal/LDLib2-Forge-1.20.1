...
public class BehaviorSettings implements Settings {
    ...
    @Getter
    @Setter
    private boolean restoreLayoutOnProjectOpen = true;
    ...
    private void onKeyDown(UIEvent event) {
        ...
        // Ctrl + Alt + S → open settings panel
        if (UIElement.isAltDown() && event.keyCode == GLFW.GLFW_KEY_S) {
            editor.openSettingsPanel();
        }

        // Ctrl + SHIFT + S → save as project
        if (UIElement.isCtrlDown() && UIElement.isShiftDown() && event.keyCode == GLFW.GLFW_KEY_S) {
            editor.saveAsProject(null); // pass null
        }

        // Ctrl + S → save project
        if (UIElement.isCtrlDown() && event.keyCode == GLFW.GLFW_KEY_S) {
